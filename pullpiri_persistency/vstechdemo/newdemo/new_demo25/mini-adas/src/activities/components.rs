...
impl EmergencyBraking {
    pub fn build(
        ...
        debug!("Initializing EmergencyBraking with persistency support");
        
        // Ensure directory exists
        if let Err(e) = std::fs::create_dir_all(crate::config::ADAS_DATA_DIR) {
            debug!("Failed to create directory {}: {:?}", crate::config::ADAS_DATA_DIR, e);
        } else {
            debug!("Directory {} created/verified", crate::config::ADAS_DATA_DIR);
        }
        ...
        let kvs = match KvsBuilder::new(InstanceId(0))
            ...
            .build() {
            Ok(kvs_instance) => {
                debug!("✅ KVS successfully initialized for EmergencyBraking");
                Some(kvs_instance)
            },
            Err(e) => {
                debug!("❌ Failed to create KVS: {:?}", e);
                None
            }
        };
    ...
    "emergency_braking_events": {{
        ...
        "confidence_score": 0.98
            ...
            if let Err(e) = kvs.set_value(key.clone(), json_data) {
                debug!("Failed to persist emergency braking data: {:?}", e);
            } else {
                // Flush to ensure data is written to disk
                if let Err(e) = kvs.flush() {
                    debug!("Failed to flush KVS data: {:?}", e);
                } else {
                    debug!("Successfully persisted emergency braking data at timestamp {}", timestamp);
                }
            }
        }
    }
...
impl Activity for EmergencyBraking {
    ...
    fn step(&mut self) {
        ...
        if let (Ok(scene), Ok(brake_instruction)) = (scene, brake_instruction) {
            const ENGAGE_DISTANCE: f64 = 35.0; // Lowered for testing
            ...
            // Persist data every 2 seconds (reduced for testing)
            let elapsed = current_time.duration_since(self.last_persist_time).unwrap_or_default();
            debug!("Persistence check: elapsed={}ms, threshold=2000ms", elapsed.as_millis());
            
            if elapsed >= Duration::from_secs(2) {
                debug!("2 seconds elapsed, persisting emergency braking data");
