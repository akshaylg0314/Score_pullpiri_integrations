...
async fn main() {
    ...
    let latest_data = Arc::new(Mutex::new(None::<ManualCarData>));
    ...
    let get_data = warp::path("data")
        ...
        .map(|latest_data: Arc<Mutex<Option<ManualCarData>>>| {
            let data = latest_data.lock().unwrap();
            let response = if let Some(ref d) = *data {
                warp::reply::json(d)
            } else {
                warp::reply::json(&serde_json::json!({"error": "No manual car data available yet"}))
            };
    ...
    // Spawn REST API server in background (port 9081 for manual)
    let rest_handle = tokio::spawn(async move {
        println!("Manual Car Data REST API running on http://localhost:9081/data");
        warp::serve(api).run(([0, 0, 0, 0], 9081)).await;
    });
    ...
    let dds_handle = tokio::spawn(async move {
        let domain_id = 100;
        let topic_name = "ManualCarData";
        let type_name = "ManualCarData";
        ...
        let reader = subscriber
            .create_datareader::<ManualCarData>(&topic, QosKind::Specific(reader_qos), NoOpListener::new(), NO_STATUS)
        ...
        println!("Manual DDS Subscriber ready - waiting for data...");
            ...
            match wait_set.wait(Duration::new(5, 0)) { // Increased from 2 to 5 seconds
                Ok(_) => {
                        ...
                        if !historical_samples.is_empty() {
                            ...
                            for sample in historical_samples {
                                if let Ok(data) = sample.data() {
                                    println!("📡 Received historical manual data: speed={}, throttle={:.1}%", 
                            ...
                            if !retry_samples.is_empty() {
                                ...
                                for sample in retry_samples {
                                    if let Ok(data) = sample.data() {
                                        println!("📡 Received historical manual data (retry): speed={}, throttle={:.1}%", 
                                            data.vehicle_speed, data.throttle_position);
                                        *latest_data_sub.lock().unwrap() = Some(data.clone());
                                    }
                                }
                    ...
                    for sample in samples {
                        if let Ok(data) = sample.data() {
                            println!("📡 Received fresh manual data: speed={}, throttle={:.1}%", 
                                data.vehicle_speed, data.throttle_position);
