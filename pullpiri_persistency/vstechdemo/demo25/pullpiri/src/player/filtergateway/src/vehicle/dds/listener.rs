...
impl TopicListener {
    ...
    async fn listener_loop(
        ...
        loop {
            ...
            // TODO: In a real DDS implementation, actual data should be received here
            // For now, do not send dummy data for test/simulation purposes
            // If there is no real DDS publisher, do not send data

            // Only execute the code below when an actual DDS message is received:
            // let dds_data = DdsData {
            //     name: data_type_name.clone(),
            //     value: received_message_json,
            ...
            // Only check if the channel is closed
            if tx.is_closed() {
                warn!("Channel closed, stopping listener for {}", topic_name);
                break;
            }
