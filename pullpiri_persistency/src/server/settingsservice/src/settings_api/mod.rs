...
async fn list_nodes(
    ...
    // Fetch from monitoring server data via persistency or direct integration
...
async fn fetch_node_logs(node_name: &str) -> Result<Vec<String>, String> {
    ...
    // This could query persistency or directly communicate with monitoring server
    monitoring_persistency::get_node_logs(node_name)
        .await
        .map_err(|e| format!("Failed to fetch node logs: {}", e))
}

async fn fetch_soc_logs(soc_id: &str) -> Result<Vec<String>, String> {
    monitoring_persistency::get_soc_logs(soc_id)
        .await
        .map_err(|e| format!("Failed to fetch SoC logs: {}", e))
}

async fn fetch_board_logs(board_id: &str) -> Result<Vec<String>, String> {
    monitoring_persistency::get_board_logs(board_id)
        .await
...
async fn fetch_container_from_monitoring_server(id: &str) -> Result<Option<ContainerInfo>, String> {
    match crate::monitoring_persistency::get_container_info(id).await {
...
async fn fetch_soc_from_monitoring_server(name: &str) -> Result<Option<SocInfo>, String> {
    match crate::monitoring_persistency::get_soc_info(name).await {
        Ok(soc) => Ok(Some(soc)),
        Err(crate::monitoring_persistency::MonitoringPersistencyError::NotFound) => Ok(None),
        Err(e) => Err(format!("PERSISTENCY error: {}", e)),
    }
...
async fn fetch_board_from_monitoring_server(name: &str) -> Result<Option<BoardInfo>, String> {
    match crate::monitoring_persistency::get_board_info(name).await {
