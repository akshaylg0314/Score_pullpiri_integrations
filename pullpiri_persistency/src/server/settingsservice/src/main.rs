...
//! - Metrics data filtering from PERSISTENCY
//! - REST API interface
//! - Schema validation
...
pub mod monitoring_persistency;
...
struct Args {
    ...
    /// PERSISTENCY endpoints (comma separated)
    #[arg(long, default_value = "localhost:2379")]
    persistency_endpoints: String,
...
async fn main() -> Result<()> {
    ...
    info!("PERSISTENCY endpoints: {}", args.persistency_endpoints);
...
async fn run_server_mode(args: Args) -> Result<()> {
    ...
    // Parse PERSISTENCY endpoints
    let persistency_endpoints: Vec<String> = args
        .persistency_endpoints
        .split(',')
...
mod tests {
    ...
    fn test_args_default_values() {
        ...
        assert_eq!(args.persistency_endpoints, "localhost:2379");
    ...
    fn test_args_custom_config_path() {
        ...
        assert_eq!(args.persistency_endpoints, "localhost:2379"); // Should remain default
    ...
    #[test]
    fn test_args_custom_persistency_endpoints() {
        let args = Args::parse_from([
            "settingsservice",
            "--persistency-endpoints",
            "persistency1:2379,persistency2:2379,persistency3:2379",
        ]);

        assert_eq!(args.persistency_endpoints, "persistency1:2379,persistency2:2379,persistency3:2379");
    ...
    fn test_args_all_custom_values() {
        ...
        assert_eq!(args.persistency_endpoints, "test-persistency:2379");
    ...
    fn test_args_mixed_long_short_flags() {
        let args = Args::parse_from([
            ...
            "/mixed/config.yaml",
            "--persistency-endpoints",
            "mixed-persistency:2379",
            "--bind-port",
            "3333",
        ]);

        assert_eq!(args.config, PathBuf::from("/mixed/config.yaml"));
        assert_eq!(args.persistency_endpoints, "mixed-persistency:2379");
    ...
    #[test]
    fn test_persistency_endpoints_parsing_single() {
    ...
    #[test]
    fn test_persistency_endpoints_parsing_multiple() {
        let endpoints = "persistency1:2379,persistency2:2379,persistency3:2379";
        let parsed: Vec<String> = endpoints.split(',').map(|s| s.trim().to_string()).collect();

        assert_eq!(parsed.len(), 3);
        assert_eq!(parsed[0], "persistency1:2379");
        assert_eq!(parsed[1], "persistency2:2379");
        assert_eq!(parsed[2], "persistency3:2379");
    }

    #[test]
    fn test_persistency_endpoints_parsing_with_spaces() {
        let endpoints = "persistency1:2379, persistency2:2379 , persistency3:2379";
        let parsed: Vec<String> = endpoints.split(',').map(|s| s.trim().to_string()).collect();
    ...
    fn test_module_declarations() {
        ...
        use crate::monitoring_persistency;
