...
//! Integration with monitoring server's persistency storage
...
/// Custom error type for monitoring persistency operations
#[derive(Debug, Error)]
pub enum MonitoringPersistencyError {
    #[error("Persistency operation error: {0}")]
    PersistencyOperation(String),
...
type Result<T> = std::result::Result<T, MonitoringPersistencyError>;

/// Generic function to store info in persistency
async fn store_info<T: Serialize>(resource_type: &str, resource_id: &str, info: &T) -> Result<()> {
    ...
    common::persistency::put(&key, &json_data)
        .await
        .map_err(|e| MonitoringPersistencyError::PersistencyOperation(e.to_string()))?;

    debug!("Stored {} for {}", resource_type, resource_id);
    Ok(())
}

/// Generic function to retrieve info from persistency
...
/// Generic function to get all items of a type from persistency
async fn get_all_info<T: DeserializeOwned>(resource_type: &str) -> Result<Vec<T>> {
    ...
    debug!("Retrieved {} {}s from persistency", items.len(), resource_type);
    Ok(items)
}

/// Generic function to delete info from persistency
...
/// Store node metadata in persistency
...
/// Store container metadata in persistency
...
/// Generic function to store metadata in persistency
...
mod tests {
    ...
    mod mock_persistency {
        ...
        pub async fn put(key: &str, value: &str) -> Result<(), String> {
            ...
            if should_fail {
                return Err("Mock persistency error".to_string());
            }
    ...
    #[test]
    fn test_monitoring_persistency_error_variants() {
        let persistency_error = MonitoringPersistencyError::PersistencyOperation("PERSISTENCY failed".to_string());
        let serialize_error = MonitoringPersistencyError::Serialize(
            serde_json::from_str::<serde_json::Value>("{invalid json").unwrap_err(),
        );
        let not_found_error = MonitoringPersistencyError::NotFound;
        let other_error = MonitoringPersistencyError::Other("Generic error".to_string());

        // Test Display implementation
        assert_eq!(persistency_error.to_string(), "Persistency operation error: PERSISTENCY failed");
        ...
        let debug_str = format!("{:?}", persistency_error);
        assert!(debug_str.contains("PersistencyOperation"));
        assert!(debug_str.contains("PERSISTENCY failed"));
    }

    #[test]
    fn test_monitoring_persistency_error_from_serde() {
        let serde_error = serde_json::from_str::<serde_json::Value>("{invalid json").unwrap_err();
        let monitoring_error = MonitoringPersistencyError::from(serde_error);

        match monitoring_error {
            MonitoringPersistencyError::Serialize(_) => (),
            _ => panic!("Expected Serialize variant"),
        }
    }

    #[test]
    fn test_monitoring_persistency_error_from_utf8() {
        let utf8_error = std::str::from_utf8(&[0x80, 0x81]).unwrap_err();
        let monitoring_error = MonitoringPersistencyError::from(utf8_error);

        match monitoring_error {
            MonitoringPersistencyError::Utf8(_) => (),
    ...
    fn test_result_type_alias() {
        ...
        let error: Result<i32> = Err(MonitoringPersistencyError::NotFound);
