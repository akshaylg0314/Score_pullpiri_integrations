...
//! PERSISTENCY storage client module
...
/// PERSISTENCY client wrapper for Settings Service
...
impl EtcdClient {
    ...
    pub async fn new(endpoints: Vec<String>) -> Result<Self, StorageError> {
        debug!("Connecting to PERSISTENCY endpoints: {:?}", endpoints);

        let client = Client::connect(endpoints, Some(ConnectOptions::new()))
            .await
            .map_err(|e| {
                StorageError::ConnectionFailed(format!("Failed to connect to PERSISTENCY: {}", e))
            })?;
