...
/// Launch REST API listener, gRPC server, and reload scenario data in persistency
pub async fn initialize() {
    // 먼저 호스트 노드를 persistency에 등록합니다.
...
/// settings.yaml에서 호스트 정보를 가져와 persistency에 등록합니다.
///
/// 이 함수는 apiserver가 시작될 때 호출되어 자신의 정보를 persistency에 등록합니다.
...
async fn register_host_node() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!(
        "Host node information registered to persistency: {} ({})",
        hostname, ip_address
    );
    Ok(())
}

/// Reload all scenario data in persistency
///
/// ### Parametets
/// * None
/// ### Description
...
async fn reload() {
    let scenarios_result = crate::artifact::data::read_all_scenario_from_persistency().await;
...
pub async fn apply_artifact(body: &str) -> common::Result<()> {
    ...
    // persistency에서 게스트 노드들의 정보를 가져와 yaml 전송
    let guest_nodes = find_guest_nodes().await;
    if guest_nodes.is_empty() {
        println!("No guest nodes found in persistency, skipping guest node deployment");
    } else {
...
/// * `body: &str` - whole yaml string of piccolo artifact
/// ### Description
/// delete artifact in persistency
