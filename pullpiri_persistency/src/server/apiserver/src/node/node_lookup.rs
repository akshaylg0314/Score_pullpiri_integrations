...
pub async fn find_node_by_simple_key() -> Option<String> {
    println!("Checking simplified node keys in persistency...");
...
/// Find a node directly from persistency using cluster/nodes/ prefix
pub async fn find_node_from_persistency() -> Option<String> {
    println!("Checking cluster/nodes/ prefix in persistency...");
    match persistency::get_all_with_prefix("cluster/nodes/").await {
        Ok(kvs) => {
...
pub async fn find_node_by_hostname(hostname: &str) -> Option<common::apiserver::NodeInfo> {
    ...
    match common::persistency::get_all_with_prefix("cluster/nodes/").await {
        Ok(kvs) => {
            for kv in kvs {
...
pub async fn get_node_ip() -> String {
    ...
    if let Some(ip) = find_node_from_persistency().await {
        return ip;
    }
...
/// 게스트 노드 정보를 persistency에서 검색하는 함수
pub async fn find_guest_nodes() -> Vec<NodeInfo> {
    println!("Finding guest nodes from persistency...");
    match persistency::get_all_with_prefix("cluster/nodes/").await {
        Ok(kvs) => {
...
mod tests {
    ...
    async fn test_find_node_by_simple_key_success() {
        // This test will pass if persistency returns data, fail if persistency is unavailable
        ...
        match result {
            ...
            None => {
                println!("No nodes found via simple key (expected if persistency unavailable or empty)");
            }
        }
    }
    ...
    async fn test_find_node_by_simple_key_no_nodes() {
        ...
        let result = find_node_by_simple_key().await;
        // Result can be either Some or None depending on persistency state
        println!("Simple key lookup result: {:?}", result.is_some());
    }

    #[tokio::test]
    async fn test_find_node_from_persistency_success() {
        let result = find_node_from_persistency().await;

        match result {
            Some(ip) => {
                assert!(!ip.is_empty());
                println!("Found IP via persistency: {}", ip);
            }
            None => {
                println!("No nodes found via persistency (expected if persistency unavailable or empty)");
            }
        }
    }

    #[tokio::test]
    async fn test_find_node_from_persistency_no_nodes() {
        // Test when kvs is empty
        let result = find_node_from_persistency().await;
        println!("Persistency lookup result: {:?}", result.is_some());
    }
    ...
    async fn test_find_node_from_manager_success() {
        ...
        match result {
            ...
            None => {
                println!("No nodes found via NodeManager (expected if persistency unavailable or empty)");
            }
        }
    }
    ...
    async fn test_add_node_to_simple_keys_success() {
        ...
        match add_node_to_simple_keys(test_ip).await {
            ...
            Err(e) => {
                println!(
                    "Failed to add node IP (expected if persistency unavailable): {}",
                    e
                );
            }
        }
    ...
    async fn test_add_node_to_simple_keys_various_ips() {
        ...
        for ip in test_ips {
            match add_node_to_simple_keys(ip).await {
                ...
                Err(e) => {
                    println!(
                        "Failed to add IP {} (expected if persistency unavailable): {}",
                        ip, e
                    );
                }
            }
    ...
    async fn test_error_handling_decode_failures() {
        ...
        // This is handled in find_node_from_persistency when decode fails
    ...
    async fn test_node_lookup_integration() {
        ...
        // Test persistency lookup
        let persistency_result = find_node_from_persistency().await;
        println!("Persistency result: {:?}", persistency_result.is_some());
    ...
    async fn test_concurrent_node_operations() {
        ...
        let tasks = vec![
            ...
            tokio::spawn(find_node_from_persistency()),
