...
mod tests {
    ...
    async fn test_get_topology_default() {
        ...
        // Test getting topology (will use default if persistency not available)
        match registry.get_topology().await {
            Ok(topology) => {
                // The topology might be default or previously set depending on persistency state
            ...
            Err(e) => {
                // Expected if persistency is not available during testing
                println!("Expected error getting topology (persistency unavailable): {}", e);
            }
        }
    }

    #[tokio::test]
    async fn test_get_topology_from_persistency() {
        let registry = NodeRegistry;
    ...
    async fn test_update_topology_embedded() {
        ...
        match registry.update_topology(test_topology.clone()).await {
            ...
            Err(e) => {
                println!("Expected error updating topology (persistency unavailable): {}", e);
            }
        }
    }
    ...
    async fn test_initialize_default_topology() {
        ...
        match registry.initialize_default_topology().await {
            ...
            Err(e) => {
                println!(
                    "Expected error initializing topology (persistency unavailable): {}",
                    e
                );
            }
        }
    ...
    async fn test_topology_with_empty_values() {
        ...
        match registry.update_topology(empty_topology.clone()).await {
            ...
            Err(e) => {
                println!(
                    "Expected error updating empty topology (persistency unavailable): {}",
                    e
                );
            }
        }
    ...
    async fn test_topology_with_large_config() {
            ...
            .await
        {
            ...
            Err(e) => {
                println!(
                    "Expected error updating large config topology (persistency unavailable): {}",
                    e
                );
            }
        }
    ...
    async fn test_topology_with_many_nodes() {
        ...
        match registry.update_topology(topology.clone()).await {
            ...
            Err(e) => {
                println!(
                    "Expected error updating many nodes topology (persistency unavailable): {}",
                    e
                );
            }
        }
    ...
    async fn test_error_handling_scenarios() {
        ...
        // Test get_topology error handling (persistency unavailable scenario is already covered)
    ...
    async fn test_topology_update_and_retrieve_cycle() {
        ...
        match registry.update_topology(test_topology.clone()).await {
            Ok(_) => {
                ...
                match registry.get_topology().await {
                    Ok(retrieved_topology) => {
                        ...
                        // depending on persistency availability
            ...
            Err(e) => {
                println!("Update failed (expected if persistency unavailable): {}", e);
            }
        }
    }
    ...
    fn test_default_topology_values() {
        // Test the exact default values used in get_topology when persistency is not available
        let default_topology = ClusterTopology {
            cluster_id: "default-cluster".to_string(),
            cluster_name: "PICCOLO Cluster".to_string(),
            r#type: TopologyType::Embedded.into(),
