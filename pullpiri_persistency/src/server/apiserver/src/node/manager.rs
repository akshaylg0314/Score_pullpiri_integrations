...
mod tests {
    ...
    async fn test_node_manager_operations() {
        ...
        // Test node registration (this will fail if persistency is not running, which is fine for test compilation)
        match manager.register_node(request).await {
            ...
            Err(e) => {
                ...
                println!("Expected persistency connection error: {}", e);
            }
        }
    }
    ...
    async fn test_register_multiple_nodes() {
        ...
        for request in requests {
            match manager.register_node(request.clone()).await {
                ...
                Err(e) => {
                    println!(
                        "Expected persistency connection error for {}: {}",
                        request.node_id, e
                    );
                }
            }
    ...
    async fn test_get_all_nodes() {
        ...
        match manager.get_all_nodes().await {
            Ok(nodes) => {
                // If persistency is available, we should get a vector (possibly empty)
                println!("Retrieved {} nodes", nodes.len());
            }
            Err(e) => {
                // Expected if persistency is not available
                println!("Expected persistency connection error: {}", e);
            }
        }
    }
    ...
    async fn test_get_nodes_alias() {
        ...
        match (&result1, &result2) {
            ...
            (Err(e1), Err(e2)) => {
                // Both should fail with same error if persistency unavailable
                println!(
                    "Both methods failed as expected when persistency unavailable: {} / {}",
                    e1, e2
                );
            }
            _ => {
    ...
    async fn test_update_heartbeat() {
        ...
        match manager.update_heartbeat("test-node-001").await {
            ...
            Err(e) => {
                println!(
                    "Expected error if node doesn't exist or persistency unavailable: {}",
                    e
                );
            }
        }
    ...
    async fn test_remove_node() {
        ...
        match manager.remove_node("test-node-001").await {
            ...
            Err(e) => {
                ...
                // or connection error if persistency unavailable
            }
        }
    }
    ...
    async fn test_remove_nonexistent_node() {
        ...
        match manager.remove_node("nonexistent-node-999").await {
            ...
            Err(e) => {
                ...
                // Should either be "Node not found" or persistency connection error
                println!(
    ...
    async fn test_node_lifecycle() {
        ...
        // Test complete node lifecycle if persistency is available
        if let Ok(token) = manager.register_node(request.clone()).await {
            ...
            println!("Persistency not available for full lifecycle test");
        }
    }
    ...
    async fn test_concurrent_operations() {
        ...
        for (i, request) in requests.into_iter().enumerate() {
            match manager.register_node(request).await {
                ...
                Err(e) => {
                    println!(
                        "Sequential registration {} failed (expected if persistency unavailable): {}",
                        i + 1,
                        e
                    );
                }
