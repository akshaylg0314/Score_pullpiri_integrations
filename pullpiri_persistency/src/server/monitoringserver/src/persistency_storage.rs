...
//! Store and retrieve monitoring data in persistency
...
async fn store_info<T: Serialize>(
    ...
    println!(
        "[PERSISTENCY] Stored the metrics for {}: {}",
...
async fn delete_info(resource_type: &str, resource_id: &str) -> common::Result<()> {
    ...
    println!(
        "[PERSISTENCY] Deleted the metrics for {}: {}",
        resource_type, resource_id
    );
    Ok(())
}
...
async fn get_all_info<T: DeserializeOwned>(resource_type: &str) -> common::Result<Vec<T>> {
    ...
    for kv in kv_pairs {
        match serde_json::from_str::<T>(&kv.value) {
            ...
            Err(e) => eprintln!(
                "[PERSISTENCY] Failed to deserialize {} {}: {}",
...
/// Store ContainerInfo in persistency - Using same pattern as others
...
pub async fn get_all_containers() -> common::Result<Vec<ContainerInfo>> {
    ...
    for kv in kv_pairs {
        match serde_json::from_str::<serde_json::Value>(&kv.value) {
            ...
            Err(e) => eprintln!("[PERSISTENCY] Failed to deserialize container {}: {}", kv.key, e),
        }
    }
...
mod tests {
    ...
    async fn test_store_and_delete_node_info() {
        ...
        // Should be Ok or error if persistency is not running
    ...
    async fn test_get_all_nodes_socs_boards_containers() {
        // These should not panic, even if persistency is empty or not running
    ...
    async fn test_store_info_and_get_info_generic() {
        ...
        // Should be Ok if persistency is running, Err otherwise
