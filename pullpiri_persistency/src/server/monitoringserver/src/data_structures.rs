...
impl DataStore {
    ...
    /// Stores NodeInfo and updates corresponding SocInfo and BoardInfo, then saves to persistency
    pub async fn store_node_info(&mut self, node_info: NodeInfo) -> Result<(), String> {
        ...
        // Store to persistency with better error handling
        let mut persistency_errors = Vec::new();

        if let Err(e) = crate::persistency_storage::store_node_info(&node_info).await {
            let error_msg = format!("Failed to store NodeInfo to persistency: {}", e);
            eprintln!("[PERSISTENCY] {}", error_msg);
            persistency_errors.push(error_msg);
        }

        if let Some(soc_info) = self.socs.get(&soc_id) {
            if let Err(e) = crate::persistency_storage::store_soc_info(soc_info).await {
                let error_msg = format!("Failed to store SocInfo to persistency: {}", e);
                eprintln!("[PERSISTENCY] {}", error_msg);
                persistency_errors.push(error_msg);
            }
        }
        ...
        if let Some(board_info) = self.boards.get(&board_id) {
            if let Err(e) = crate::persistency_storage::store_board_info(board_info).await {
                let error_msg = format!("Failed to store BoardInfo to persistency: {}", e);
                eprintln!("[PERSISTENCY] {}", error_msg);
                persistency_errors.push(error_msg);
            }
        }

        // Log warning if persistency operations failed but don't fail the entire operation
        if !persistency_errors.is_empty() {
            eprintln!(
                "[PERSISTENCY] Warning: {} persistency operations failed",
                persistency_errors.len()
            );
        }
    ...
    /// Stores ContainerInfo to memory and persistency
    pub async fn store_container_info(
        ...
        // Store to persistency with error handling
        if let Err(e) = crate::persistency_storage::store_container_info(&container_info).await {
            eprintln!(
                "[PERSISTENCY] Warning: Failed to store ContainerInfo to persistency: {}",
                e
            );
    ...
    /// Stores ContainerInfo to memory and persistency, with explicit node association
    ...
    /// Retrieves ContainerInfo from memory, fallback to persistency
    pub async fn get_container_info(&self, container_id: &str) -> Result<ContainerInfo, String> {
        ...
        // Fallback to persistency
        match crate::persistency_storage::get_container_info(container_id).await {
            Ok(container_info) => Ok(container_info),
            Err(e) => Err(format!("Container not found in memory or persistency: {}", e)),
        }
    }
    ...
    /// Removes container from memory and persistency
    pub async fn remove_container_info(&mut self, container_id: &str) -> Result<(), String> {
        // Remove from memory
        self.containers.remove(container_id);

        // Remove from persistency
        if let Err(e) = crate::persistency_storage::delete_container_info(container_id).await {
            eprintln!(
                "[PERSISTENCY] Warning: Failed to delete ContainerInfo from persistency: {}",
                e
            );
        }
    ...
    /// Load all containers from persistency into memory (useful for initialization)
    pub async fn load_containers_from_persistency(&mut self) -> Result<(), String> {
        match crate::persistency_storage::get_all_containers().await {
            Ok(containers) => {
                for container in containers {
                    self.containers.insert(container.id.clone(), container);
                }
                println!(
                    "[DataStore] Loaded {} containers from persistency",
                    self.containers.len()
                );
                Ok(())
            }
            Err(e) => {
                eprintln!("[PERSISTENCY] Warning: Failed to load containers from persistency: {}", e);
    ...
    /// ADD THIS METHOD for cleanup with persistency deletion
    pub async fn cleanup_node_containers(
        ...
        for container_id in containers_to_remove {
            ...
            if let Err(e) = crate::persistency_storage::delete_container_info(&container_id).await {
                eprintln!(
                    "[PERSISTENCY] Warning: Failed to delete container {} from persistency: {}",
                    container_id, e
                );
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_store_node_info_success_and_persistency_error() {
        let mut ds = DataStore::new();
    ...
    #[tokio::test]
    async fn test_store_node_info_persistency_error_handling() {
        // This test ensures that persistency_errors are handled gracefully (no panic, still Ok)
        let mut ds = DataStore::new();
        let node = sample_node("node1", "192.168.10.201");
        // persistency_storage is expected to fail in test, but store_node_info should still return Ok
    ...
    fn test_store_container_info_and_with_node() {
        ...
        // store_container_info (persistency call will fail but shouldn't panic)
        let rt = tokio::runtime::Runtime::new().unwrap();
    ...
    #[test]
    fn test_get_container_info_memory_and_persistency_fallback() {
        let mut ds = DataStore::new();
        let container = sample_container("c1", "container1");
        ds.containers.insert("c1".to_string(), container.clone());
        ...
        // Should fail for non-existent container (persistency fallback will fail)
    ...
    #[test]
    fn test_load_containers_from_persistency_handles_error() {
        let mut ds = DataStore::new();
        let rt = tokio::runtime::Runtime::new().unwrap();
        // Should not panic even if persistency fails
        let result = rt.block_on(ds.load_containers_from_persistency());
        assert!(result.is_ok());
    }
