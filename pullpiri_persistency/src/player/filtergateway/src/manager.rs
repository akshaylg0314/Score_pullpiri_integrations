...
impl FilterGatewayManager {
    ...
    ///
    ///
    /// This function reads all scenarios from persistency and subscribes to the necessary vehicle data topics.
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        let persistency_scenario = Self::read_all_scenario_from_persistency()
            .await
            .unwrap_or_default();

        for scenario in persistency_scenario {
    ...
    /// Read all scenario yaml string in persistency
    ///
    /// ### Parameters
    /// * None
    /// ### Return
    /// * `Result<Vec<String>>` - `Ok(_)` contains scenario yaml string vector
    async fn read_all_scenario_from_persistency() -> common::Result<Vec<String>> {
...
mod tests {
    ...
    /// Test scenario initialization from persistency
    #[tokio::test]
    async fn test_initialize_with_scenario_subscription_error() {
        ...
        impl MockInitManager {
            async fn mock_initialize(&self) -> Result<()> {
                ...
                // Simulate scenarios from persistency
                let scenarios = vec!["
apiVersion: v1
kind: Scenario
metadata:
