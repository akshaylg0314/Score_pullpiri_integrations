...
impl StateManagerManager {
    ...
    ///
    /// # Future Enhancements
    /// - Load persisted resource states from storage (persistency, database)
    ...
    async fn process_state_change(&self, state_change: StateChange) {
        ...
        //    - Update resource state in persistent storage (persistency cluster, database)
        ...
        if result.is_success() {
            ...
            // 🔍 COMMENT 6: Save scenario state changes to PERSISTENCY
            // StateManager receives state change requests from FilterGateway, ActionController, and PolicyManager
            // and saves the scenario state transitions to PERSISTENCY for persistence
            if resource_type == ResourceType::Scenario {
                println!("💾 SCENARIO STATE PERSISTENCE: StateManager PERSISTENCY Storage");
                println!("   📋 Scenario: {}", state_change.resource_name);
                println!("   🔄 Final State: {}", new_state_str);
                println!("   🔍 Reason: Successful state transition completed");

                let persistency_key = format!("/scenario/{}/state", state_change.resource_name);
                let persistency_value = new_state_str;

                println!("   📤 Saving to PERSISTENCY:");
                println!("      • Key: {}", persistency_key);
                println!("      • Value: {}", persistency_value);
                println!("      • Operation: common::persistency::put()");

                if let Err(e) = common::persistency::put(&persistency_key, persistency_value).await {
                    println!("   ❌ Failed to save scenario state to PERSISTENCY: {:?}", e);
                } else {
                    println!(
                        "   ✅ Successfully saved scenario state to PERSISTENCY: {} → {}",
                        persistency_key, persistency_value
                    );
                }
            }
    ...
    /// 4. Update model states in PERSISTENCY if transitions occur
        ...
        for (model_name, containers) in model_containers {
            ...
            if transition_result.is_success() {
                ...
                if state_changed {
                    ...
                    // Save the new model state to PERSISTENCY
                    drop(state_machine); // Release the lock before async operation
                    if let Err(e) = self
                        .save_model_state_to_persistency(&model_name, new_model_state)
                        .await
                    {
                        println!("    Failed to save model state to PERSISTENCY: {:?}", e);
                    } else {
                        println!("    Successfully saved model state to PERSISTENCY");
    ...
    /// Saves model state to PERSISTENCY using the format specified in the documentation
    async fn save_model_state_to_persistency(
        &self,
        model_name: &str,
        ...
        println!("    Saving to PERSISTENCY - Key: {}, Value: {}", key, value);
    ...
    /// Saves package state to PERSISTENCY using the format specified in the Korean documentation
    ///
    /// Format: /package/{package_name}/state -> state_value (e.g., "running", "degraded", "error")
    async fn save_package_state_to_persistency(
        &self,
        package_name: &str,
        ...
        println!(
            "    Saving package state to PERSISTENCY - Key: {}, Value: {}",
            key, value
        );
        ...
        for package_name in packages {
                ...
                .await
            {
                Ok((state_changed, new_state)) => {
                    ...
                    if state_changed {
                        // Save new state to PERSISTENCY
                        if let Err(e) = self
                            .save_package_state_to_persistency(&package_name, new_state)
                            .await
                        {
                            println!("      Failed to save package state: {:?}", e);
                            continue;
    ...
    async fn find_scenario_for_package(
        ...
        // Get all scenarios from PERSISTENCY
        match common::persistency::get_all_with_prefix("Scenario/").await {
            ...
            Err(e) => {
                println!("      Failed to get scenarios from PERSISTENCY: {:?}", e);
                Err(format!("Failed to get scenarios from PERSISTENCY: {:?}", e))
            }
        }
    }
...
//    - Integration with persistency or database for state persistence
...
mod integration_tests {
    ...
    async fn test_statemanager_actioncontroller_communication() {
        ...
        // Create test scenario and package data in persistency
        let scenario_yaml = r#"
apiVersion: v1
kind: Scenario
metadata:
...
"#;
        ...
        common::persistency::delete("Scenario/test-communication-scenario")
            .await
            .expect("Failed to cleanup scenario from persistency");

        common::persistency::delete("Package/test-communication-package")
            .await
            .expect("Failed to cleanup package from persistency");
