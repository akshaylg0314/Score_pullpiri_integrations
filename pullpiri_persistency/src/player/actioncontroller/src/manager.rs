...
impl ActionControllerManager {
    ...
    ///
    /// Retrieves node information from persistency to determine if it is a bluechi or nodeagent node.
    ///
    /// # Arguments
    ///
    ...
    async fn get_node_role_from_persistency(&self, node_name: &str) -> Result<String> {
        ...
        let node_ip = match common::persistency::get(&node_info_key).await {
            ...
            Err(e) => {
                println!(
                    "Warning: Failed to get IP for node '{}' from persistency: {}",
                    node_name, e
                );
        ...
        let node_json = match common::persistency::get(&cluster_node_key).await {
            ...
            Err(e) => {
                println!(
                    "Warning: Failed to get details for node '{}' from persistency: {}",
                    node_name, e
                );
                // settings.yaml 파일의 호스트 정보 사용
                let config = common::setting::get_config();
    ...
    pub async fn trigger_manager_action(&self, scenario_name: &str) -> Result<()> {
        ...
        let persistency_scenario_key = format!("Scenario/{}", scenario_name);
        let scenario_str: String = match common::persistency::get(&persistency_scenario_key).await {
            Ok(value) => value,
            Err(e) => {
        ...
        let package_str = match common::persistency::get(&persistency_package_key).await {
            Ok(value) => value,
            Err(e) => {
                return Err(format!("Package key '{}' not found: {}", persistency_package_key, e).into());
            }
        };
        ...
        // To Do.. network, node yaml extract from persistency.
        let persistency_network_key = format!("Network/{}", scenario_name);
        let network_str = (common::persistency::get(&persistency_network_key).await).ok();

        let persistency_node_key = format!("Node/{}", scenario_name);
        let node_str = (common::persistency::get(&persistency_node_key).await).ok();

        // 각 모델의 노드 정보를 persistency에서 확인
        ...
        for mi in package.get_models() {
            ...
            // 노드 역할 정보가 캐시에 없으면 persistency에서 가져옴
            if !node_roles.contains_key(&model_node) {
                match self.get_node_role_from_persistency(&model_node).await {
                    ...
                    Err(e) => {
                        println!(
                            "Warning: Failed to get role for node '{}' from persistency: {}",
                            model_node, e
                        );
                        // persistency에서 정보를 찾을 수 없으면 설정 파일 정보 확인
...
mod tests {
    ...
    async fn test_trigger_manager_action_with_valid_data() {
        // Insert mock Scenario YAML into persistency
...
"#,
        ...
        // Insert mock Package YAML into persistency
        common::persistency::put(
